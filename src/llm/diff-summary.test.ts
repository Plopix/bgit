...
function fakeAnthropic() {
    const provider = ((modelId: string) => ({ modelId })) as unknown as ReturnType<typeof createAnthropic>;
    return provider;
}
...
describe('summariseFileDiff', () => {
    ...
    it('returns the text produced by generateText', async () => {
        const result = await summariseFileDiff(fakeAnthropic(), 'claude-opus-4-6', 'src/index.ts', '- old\n+ new', '');

        expect(result).toBe('mocked summary');
    });

    it('calls generateText with the correct model', async () => {
        await summariseFileDiff(fakeAnthropic(), 'claude-opus-4-6', 'src/index.ts', '- old\n+ new', '');
...
+ const x = 2;`;
    ...
    it('propagates errors from generateText', async () => {
        ...
        await expect(summariseFileDiff(fakeAnthropic(), 'claude-opus-4-6', 'a.ts', '+ x', '')).rejects.toThrow(
            'API rate limit',
        );
    });
