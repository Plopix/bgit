    ...
    try {
      ...
      for (const commit of commits) {
        if (!commit.hash || !commit.author || !commit.date || !commit.message) {
          throw new Error(
            "Each commit must have hash, author, date, and message fields."
          );
        }
      }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <label
          htmlFor="git-json-input"
          className="text-sm font-medium text-foreground"
        >
          Paste your git log JSON
        </label>
        <Button variant="ghost" size="sm" onClick={handleLoadExample} className="text-xs text-muted-foreground hover:text-foreground">
          Load example
        </Button>
      </div>
      <textarea
